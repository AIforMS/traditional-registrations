...
from get_metrics import dice_coeff, Get_Jac, jacobian_determinant


parser = argparse.ArgumentParser()

# dataset args
parser.add_argument("-dataset", choices=["tcia", "lpba", "chaos", "ct-mr", "mr-ct"], default='tcia', required=False)
parser.add_argument("-img_folder", help="training CTs dataset folder",
...
parser.add_argument("-mov_numbers", 
                    help="list of integers indicating which scans to use, i.e. \"1 2 3\" ",
                    default="2 12 17 29",
                    type=lambda s: [n for n in s.split()])
parser.add_argument("-fix_numbers", type=str, 
                    help="integers indicating which scan to use, i.e. 1",
                    default="02")
parser.add_argument("-mov_name",
                    help="prototype scan filename i.e. pancreas_ct?.nii.gz",
                    default='S?.delineation.skullstripped.nii.gz')  # pancreas_ct?.nii.gz
parser.add_argument("-fix_name",
                    help="prototype scan filename i.e. pancreas_ct?.nii.gz",
                    default='S?.delineation.skullstripped.nii.gz')  # pancreas_ct?.nii.gz
parser.add_argument("-mov_label", help="prototype segmentation name i.e. label_ct?.nii.gz",
                    default="S?.delineation.structure.label.nii.gz")
parser.add_argument("-fix_label", help="prototype segmentation name i.e. label_ct?.nii.gz",
                    default="S?.delineation.structure.label.nii.gz")
parser.add_argument("-trans_typ", help="filename (without extension) for output",
                    default="SyN")
parser.add_argument("-output", help="filename (without extension) for output",
                    default="./results/ANTs_SyN")
...
def ants_reg(mov_img_p,
    ...
    for idx, (mov_img, mov_label) in enumerate(zip(mov_img_lst, mov_label_lst)):
        mov_name = re.findall(r'\d+', os.path.split(mov_img)[-1])[0]
        fix_name = re.findall(r'\d+', os.path.split(fix_img_p)[-1])[0]
        save_name = f"{mov_name}_{fix_name}"
        ...
        dice_one_val = dice_coeff(warped_label.numpy(single_components=False), m_label.numpy(single_components=False),
                                  logger=logger)
        np.set_printoptions(precision=3, suppress=True)
        logger.info(f"{save_name} : register DSC: dice list {dice_one_val}, DSC {dice_one_val.mean()}")
...
if __name__ == "__main__":
    ...
    # 固定图像为一个文件路径，不是列表
    fix_img_p = os.path.join(args.img_folder, args.fix_name.replace('?', args.fix_number))
    fix_label_p = os.path.join(args.label_folder, args.fix_label.replace('?', args.fix_number))

    # 浮动图像路径列表
    mov_img_p = [os.path.join(args.img_folder, args.mov_name.replace('?', i)) for i in args.mov_numbers]
    mov_label_p = [os.path.join(args.label_folder, args.mov_label.replace('?', i)) for i in args.mov_numbers]
